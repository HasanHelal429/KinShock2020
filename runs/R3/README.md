...
> Negative control (n_e0 = 0). Clone of R1_warm with numerics.ppc.ambient = 0 so no ambient macroparticles load; B0 and all scales stay identical to R1_warm. Confirms no ambient structures / reflected-ion beam / shock form without an upstream medium (Fig. 9).

**tier** Core · **deck** `inputs_kinshock_R3` · **paper** Schaeffer et al., Phys. Plasmas 27, 042901 (2020), Fig. 9
...
Edit these; never the deck. Regenerate with `python scripts/make_inputs.py runs/R3`.
...
| ppc | piston 100, ambient 0 | `numerics.ppc` |
...
| `inputs_kinshock_R3` | generated deck (tracked, **never hand-edit**) |
