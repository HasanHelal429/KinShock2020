...
> Matched control for R1_paper_phys: same truncated (t*wci0 = 0.389) and ppc-25 setup, but keeping Table I's lambda_ab = 20 dial (lnLambda = 1.223e5). Strongly collisional (mfp_ei,ab = 20 d_e,ab, nu_ei*dt = 3.4e-3, upstream mfp_ii/d_i0 = 0.0150). Exists so the physical-lnLambda test is compared against identical particle noise rather than against R1_paper's ppc 100.

**tier** Full · **deck** `inputs_kinshock_R1_paper_dial` · **paper** Schaeffer et al., Phys. Plasmas 27, 042901 (2020), Table I + Sec. II
...
Edit these; never the deck. Regenerate with `python scripts/make_inputs.py runs/R1_paper_dial`.
...
| `inputs_kinshock_R1_paper_dial` | generated deck (tracked, **never hand-edit**) |
