...
> Smoke test of the R1 setup on a tiny grid (domain +-500 d_e, 2000 cells, 2000 steps). Verifies the deck + heater/injector + analysis pipeline before committing to Core/Full-tier runs.

**tier** Smoke · **deck** `inputs_kinshock_R0` · **paper** Schaeffer et al., Phys. Plasmas 27, 042901 (2020)
...
Edit these; never the deck. Regenerate with `python scripts/make_inputs.py runs/R0`.
...
| boundary lo / hi | periodic | `geometry.boundary` |
...
| n_cell | 2000 | domain / dz (halved when one_sided) |
...
| `inputs_kinshock_R0` | generated deck (tracked, **never hand-edit**) |
