...
> Half-domain (one-sided) smoke test: same R0 physics on [0, 500 d_e] with a reflecting wall at z=0 (1000 cells vs R0's 2000). Validation target: reproduce R0's z>=0 loaded state + operator balance at half the cost.

**tier** Smoke · **deck** `inputs_kinshock_R0_half` · **paper** Schaeffer et al., Phys. Plasmas 27, 042901 (2020)
...
Edit these; never the deck. Regenerate with `python scripts/make_inputs.py runs/R0_half`.
...
| dz [m] | 0.00265705 | dz_over_de * d_e |
| dt [s] | 6.64722e-12 | CFL-limited |
| dt*omega_pe | 0.375 | dt * omega_pe |
| n_cell | 1000 | domain / dz (halved when one_sided) |
| steps per 1/omega_ci0 | 26666.7 | 1/(omega_ci0*dt) |
| run length [1/omega_ci0] | 0.075 | max_step * dt * omega_ci0 |
...
| `inputs_kinshock_R0_half` | generated deck (tracked, **never hand-edit**) |
