...
> eps ladder rung 2/5: T_e,ab = 1486 eV (eps = v_te,ab/c = 0.0539), B0 = 12.494 T from beta_ab = 1150. dz/lambda_D,0 = 0.379 and ppc 100 held identical to ss_dz16_ppc100, so eps is the only parameter that moves. Early-formation resolution sweep point; see runs/S_phase/README.md.

**tier** Sweep · **deck** `inputs_kinshock_es_1p5keV` · **paper** Schaeffer et al., Phys. Plasmas 27, 042901 (2020), Table I + Sec. II
...
Edit these; never the deck. Regenerate with `python scripts/make_inputs.py runs/es_1p5keV`.
...
| heater theta_e,ab | 0.00290803 | `plasma.piston.theta_e_heat` |
| piston init theta_e | 6.1873e-05 | `plasma.piston.theta_e_init` |
| piston init theta_i | 6.1873e-07 | `plasma.piston.theta_i_init` |
| ambient density / n0 | 0.008 | `plasma.ambient.density_over_n0` |
| ambient theta_0 | 6.1873e-05 | `plasma.ambient.theta_0` |
| field orientation | perpendicular | `field.orientation` |
| **B0 [T]** | 12.4938 | `field.B0_tesla` |
...
| C_s,ab / c | 0.00539261 | sqrt(theta_e,ab/mass_ratio) |
| t_ab [s] | 1.34194e-12 | d_i,ab / C_s,ab |
| n_amb [m^-3] | 4.8e+24 | density_over_n0 * n0 |
| d_i0 [m] | 2.42554e-05 | c/omega_pi(n_amb) |
| d_i0 / d_e | 111.803 | derived |
| **omega_ci0 [rad/s]** | 2.19744e+10 | **q_e*B0/m_i — B0 only, no n_amb** |
| 1/omega_ci0 [s] | 4.55074e-11 | 1/omega_ci0 |
| **v_A / c** | 0.00177789 | **B0/sqrt(mu0*n_amb*m_i) — DERIVED** |
| rho_i0 / d_e | 1162.76 | v_p/omega_ci0 / d_e |
| v_p / c (model) | 0.0184901 | config model.vp_over_c |
| v_sh / c (model) | 0.024806 | model.vsh_over_Csab * C_s,ab |
| M_A | 13.9525 | v_sh/v_A |
| M_ms | 12.7595 | v_sh/sqrt(v_A^2+C_s0^2) |
| beta_ab | 1150 | mu0*n0*T_e,ab/B0^2 |
| beta_0 | 0.195745 | mu0*n_amb*T_0/B0^2 |
| dz [m] | 7.233e-09 | dz_over_de * d_e |
| dt [s] | 1.8095e-17 | CFL-limited |
| dt*omega_pe | 0.025005 | dt * omega_pe |
| n_cell | 40312 | domain / dz (halved when one_sided) |
| steps per 1/omega_ci0 | 2.51492e+06 | 1/(omega_ci0*dt) |
| run length [1/omega_ci0] | 0.301831 | max_step * dt * omega_ci0 |
| T_e,ab [eV] | 1486 | theta_e,ab * m_e c^2 |
| lnLambda (used) | 122.242 | units.coulomb_log_for(collisions.target) |
| lnLambda (physical) | 7.38211 | NRL at (n0, T_e,ab) |
| nu_ei,ab [1/s] | 3.72595e+12 | NRL electron-ion |
| nu_ei*dt | 6.74211e-05 | must be << 1 |
...
| C_s,ab / c | 0.00539261 | 0.03 | 0.180x (-82.0%) **OFF** | theta_e_heat recalibrated off the paper's 0.092 |
| piston speed v_p / c | 0.0184901 | 0.104 | 0.178x (-82.2%) **OFF** |  |
...
| `inputs_kinshock_es_1p5keV` | generated deck (tracked, **never hand-edit**) |
