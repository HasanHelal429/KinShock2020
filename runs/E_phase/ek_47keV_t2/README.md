...
> PRODUCTION 470 eV run at the adopted numerics (dz/lambda_D = 0.758, shape 3, filter 8, ppc 100), on a 34.2 d_i0 domain to t* = 2.0 so shock formation (t*_1 ~ 1.35-1.41) and ~0.6 gyroperiods beyond it are all pre-arrival. Grown from ss_dz8_ppc100_s3f8, whose numerics it carries unchanged; only the domain (12.02 -> 34.21 d_i0) and the clock (0.302 -> 2.000 /w_ci0) differ. See runs/S_phase/README.md for the numerics choice and RESULTS 2026-08-17 for why the remaining difference from R1_paper is physics, not resolution.

**tier** Sweep · **deck** `inputs_kinshock_ek_47keV_t2` · **paper** Schaeffer et al., Phys. Plasmas 27, 042901 (2020), Table I + Sec. II
...
Edit these; never the deck. Regenerate with `python scripts/make_inputs.py runs/ek_47keV_t2`.
...
| heater theta_e,ab | 0.0920002 | `plasma.piston.theta_e_heat` |
| piston init theta_e | 0.00195745 | `plasma.piston.theta_e_init` |
| piston init theta_i | 1.95745e-05 | `plasma.piston.theta_i_init` |
| ambient density / n0 | 0.008 | `plasma.ambient.density_over_n0` |
| ambient theta_0 | 0.00195745 | `plasma.ambient.theta_0` |
...
| domain halfwidth [d_e] | 6149.2 | `geometry.domain_halfwidth_de` |
| dz [d_e] | 0.37495 | `geometry.dz_over_de` |
| boundary lo / hi | lo symmetry, hi open | `geometry.boundary` |
| CFL | 0.75 | `numerics.cfl` |
| particle shape | 3 | `numerics.particle_shape` |
| max_step | 79515 | `numerics.max_step` |
...
| t_ab [s] | 2.38582e-13 | d_i,ab / C_s,ab |
...
| 1/omega_ci0 [s] | 8.09072e-12 | 1/omega_ci0 |
...
| dz [m] | 8.13444e-08 | dz_over_de * d_e |
| dt [s] | 2.03502e-16 | CFL-limited |
| dt*omega_pe | 0.281213 | dt * omega_pe |
| n_cell | 16400 | domain / dz (halved when one_sided) |
| steps per 1/omega_ci0 | 39757.5 | 1/(omega_ci0*dt) |
| run length [1/omega_ci0] | 2 | max_step * dt * omega_ci0 |
| T_e,ab [eV] | 47012 | theta_e,ab * m_e c^2 |
| lnLambda (used) | 122349 | units.coulomb_log_for(collisions.target) |
| lnLambda (physical) | 10.8364 | NRL at (n0, T_e,ab) |
| nu_ei,ab [1/s] | 2.09571e+13 | NRL electron-ion |
| nu_ei*dt | 0.00426481 | must be << 1 |
...
| `inputs_kinshock_ek_47keV_t2` | generated deck (tracked, **never hand-edit**) |
