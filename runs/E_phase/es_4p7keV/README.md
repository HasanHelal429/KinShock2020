...
**tier** Sweep · **deck** `inputs_kinshock_es_4p7keV` · **paper** Schaeffer et al., Phys. Plasmas 27, 042901 (2020), Table I + Sec. II
...
Edit these; never the deck. Regenerate with `python scripts/make_inputs.py runs/es_4p7keV`.
...
| heater theta_e,ab | 0.00919767 | `plasma.piston.theta_e_heat` |
| piston init theta_e | 0.000195695 | `plasma.piston.theta_e_init` |
| piston init theta_i | 1.95695e-06 | `plasma.piston.theta_i_init` |
| ambient density / n0 | 0.008 | `plasma.ambient.density_over_n0` |
| ambient theta_0 | 0.000195695 | `plasma.ambient.theta_0` |
| field orientation | perpendicular | `field.orientation` |
| **B0 [T]** | 22.2196 | `field.B0_tesla` |
...
| C_s,ab / c | 0.00959045 | sqrt(theta_e,ab/mass_ratio) |
| t_ab [s] | 7.5456e-13 | d_i,ab / C_s,ab |
| n_amb [m^-3] | 4.8e+24 | density_over_n0 * n0 |
| d_i0 [m] | 2.42554e-05 | c/omega_pi(n_amb) |
| d_i0 / d_e | 111.803 | derived |
| **omega_ci0 [rad/s]** | 3.90802e+10 | **q_e*B0/m_i — B0 only, no n_amb** |
| 1/omega_ci0 [s] | 2.55884e-11 | 1/omega_ci0 |
| **v_A / c** | 0.00316188 | **B0/sqrt(mu0*n_amb*m_i) — DERIVED** |
| rho_i0 / d_e | 1162.76 | v_p/omega_ci0 / d_e |
| v_p / c (model) | 0.0328835 | config model.vp_over_c |
| v_sh / c (model) | 0.0441161 | model.vsh_over_Csab * C_s,ab |
| M_A | 13.9525 | v_sh/v_A |
| M_ms | 12.7595 | v_sh/sqrt(v_A^2+C_s0^2) |
| beta_ab | 1150 | mu0*n0*T_e,ab/B0^2 |
| beta_0 | 0.195745 | mu0*n_amb*T_0/B0^2 |
| dz [m] | 1.28652e-08 | dz_over_de * d_e |
| dt [s] | 3.21852e-17 | CFL-limited |
| dt*omega_pe | 0.0444758 | dt * omega_pe |
| n_cell | 22664 | domain / dz (halved when one_sided) |
| steps per 1/omega_ci0 | 795035 | 1/(omega_ci0*dt) |
| run length [1/omega_ci0] | 0.301831 | max_step * dt * omega_ci0 |
| T_e,ab [eV] | 4700 | theta_e,ab * m_e c^2 |
| lnLambda (used) | 1222.87 | units.coulomb_log_for(collisions.target) |
| lnLambda (physical) | 8.53359 | NRL at (n0, T_e,ab) |
| nu_ei,ab [1/s] | 6.62638e+12 | NRL electron-ion |
| nu_ei*dt | 0.000213272 | must be << 1 |
...
| C_s,ab / c | 0.00959045 | 0.03 | 0.320x (-68.0%) **OFF** | theta_e_heat recalibrated off the paper's 0.092 |
| piston speed v_p / c | 0.0328835 | 0.104 | 0.316x (-68.4%) **OFF** |  |
...
| `inputs_kinshock_es_4p7keV` | generated deck (tracked, **never hand-edit**) |
