...
> eps ladder rung 4/5: T_e,ab = 14860 eV (eps = v_te,ab/c = 0.1705), B0 = 39.509 T from beta_ab = 1150. dz/lambda_D,0 = 0.379 and ppc 100 held identical to ss_dz16_ppc100, so eps is the only parameter that moves. Early-formation resolution sweep point; see runs/S_phase/README.md.

**tier** Sweep · **deck** `inputs_kinshock_es_15keV` · **paper** Schaeffer et al., Phys. Plasmas 27, 042901 (2020), Table I + Sec. II
...
Edit these; never the deck. Regenerate with `python scripts/make_inputs.py runs/es_15keV`.
...
| heater theta_e,ab | 0.0290803 | `plasma.piston.theta_e_heat` |
| piston init theta_e | 0.00061873 | `plasma.piston.theta_e_init` |
| piston init theta_i | 6.1873e-06 | `plasma.piston.theta_i_init` |
| ambient density / n0 | 0.008 | `plasma.ambient.density_over_n0` |
| ambient theta_0 | 0.00061873 | `plasma.ambient.theta_0` |
| field orientation | perpendicular | `field.orientation` |
| **B0 [T]** | 39.509 | `field.B0_tesla` |
...
| C_s,ab / c | 0.0170529 | sqrt(theta_e,ab/mass_ratio) |
| t_ab [s] | 4.24359e-13 | d_i,ab / C_s,ab |
| n_amb [m^-3] | 4.8e+24 | density_over_n0 * n0 |
| d_i0 [m] | 2.42554e-05 | c/omega_pi(n_amb) |
| d_i0 / d_e | 111.803 | derived |
| **omega_ci0 [rad/s]** | 6.94893e+10 | **q_e*B0/m_i — B0 only, no n_amb** |
| 1/omega_ci0 [s] | 1.43907e-11 | 1/omega_ci0 |
| **v_A / c** | 0.00562219 | **B0/sqrt(mu0*n_amb*m_i) — DERIVED** |
| rho_i0 / d_e | 1162.76 | v_p/omega_ci0 / d_e |
| v_p / c (model) | 0.0584708 | config model.vp_over_c |
| v_sh / c (model) | 0.0784435 | model.vsh_over_Csab * C_s,ab |
...
| C_s,ab / c | 0.0170529 | 0.03 | 0.568x (-43.2%) **OFF** | theta_e_heat recalibrated off the paper's 0.092 |
| piston speed v_p / c | 0.0584708 | 0.104 | 0.562x (-43.8%) **OFF** |  |
...
| `inputs_kinshock_es_15keV` | generated deck (tracked, **never hand-edit**) |
