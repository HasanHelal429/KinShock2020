# Run `R1_core_half_sym`

> One-sided R1_core with the rotational-symmetry (pi-rotation about B0) wall at z=0 instead of specular reflection. Full R1 physics on the +z half [0, 3600 d_e] (12000 cells, 125000 steps ~ 2.8 wci0^-1). Controlled A/B vs R1_core_half (specular) and R1_core (full domain).

**tier** Core · **deck** `inputs_kinshock_R1_core_half_sym` · **paper** Schaeffer et al., Phys. Plasmas 27, 042901 (2020), Table I
...
Edit these; never the deck. Regenerate with `python scripts/make_inputs.py runs/R1_core_half_sym`.
...
| `inputs_kinshock_R1_core_half_sym` | generated deck (tracked, **never hand-edit**) |
